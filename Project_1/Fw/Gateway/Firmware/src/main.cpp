...
const char *mainMenu[] = {"Time Setting", "Screen Setting", "Internet Setting", "Schedule setup"};
...
void handleApiStatus()
{
    ...
    for (int i = 0; i < nodeCount; i++)
    {
        if (nodes[i].id > 0)
        {
            ...
            json += "\"timeString\":\"" + slaves[slaveIdx].timeString + "\"";
            json += "}";
            renderedNodes++;
        }
    }
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else if (menuLevel == 2)
            {
                ...
                else if (submenuSelected == 1) // Screen Flip
                {
                    u8g2.setFont(u8g2_font_ncenB08_tr);
...
void ioTask(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (inMenu)
        {
            ...
            else if (menuLevel == 2 && submenuSelected == 1) // Screen Flip
            {
                if (digitalRead(BT_UP_PIN) == LOW || digitalRead(BT_DWN_PIN) == LOW)
                {
                    buzzerBeep(1000, 50);
                    screenFlip = !screenFlip;
                    while (digitalRead(BT_UP_PIN) == LOW || digitalRead(BT_DWN_PIN) == LOW)
                        vTaskDelay(10);
                }
                if (digitalRead(BT_SEL_PIN) == LOW)
                {
